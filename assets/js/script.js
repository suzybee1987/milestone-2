  ...
  I have borrowed code from this tutorial to assist with creating the quiz including pagination
  and I have also personalised some of the code for my own styling and created 
  event listeners and some functions of my own
  */
  ...
  //   characters for the quiz and their descriptions and books
  const characters = {
  	death: {
  		name: "Death",
  ...
  function buildQuiz() {
  	// answer keys in here
  	let userAnswers = [];
  	//   automatically muted audio for better UX 
  	audio.muted = true;
  	...
  	function startGame() {
  		...
  		questionList.forEach(
  			(currentQ, qNumber) => {
  				...
  				for (let option in currentQ.answers) {
  					answers.push(
  		...
  		function getResults(value) {
  			...
  			//   end of help from Sean 

  			let books = 0;
  			let description = 0;
  			let image = 0;

  			// if statement to determine which character has most votes 
  			...
  			//   call function to show chosenAnswer 
  			showResults(chosenAnswer);
  		}

  		// pagination when answer button clicked - trigger by click handler when user clicks answer
  		...
  		$('.answer').click(function () {
  			...
  			//   adds chosen answer to userAnswers 
  			userAnswers.push(this.value);

  			//   loads next question 
  			showNextSlide();

  			//   clicked class enlarges the button when clicked using css
  			$(this).addClass('clicked');

  			// if volume muted do nothing 
  			if (!audio) return;

  			//   reset time to 0 seconds in case sound overlaps next question being answered 
  			audio.currentTime = 0;
  		...
  		//   this function closes the navbar menu on md and sm devices when a link is clicked 
